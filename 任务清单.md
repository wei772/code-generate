...
* ~~支持命令行~~
  ...
  * ~~java -jar code-generate.jar 错误: 无法初始化主类 cn.garden.generate.CommandLineApplication  
...
* ~~以插件的方式连接Oracle~~
  * ~~指定Main运行~~
  * ~~java -cp  lib\*;code-generate.jar  cn.garden.generate.CommandLineApplication  -c=entityDemo  -e=jdbc  -b=D:\source\code-generate\src\test\resources\generateResult  -k=cn.cli.jdbc~~
    * ~~在jar包同级创建目录lib~~
    * ~~**这种方法是最佳的**~~
...
* ~~[重构] 根包名不能用cn.garden，用cn.garden.generate，要不然太容易冲突~~
